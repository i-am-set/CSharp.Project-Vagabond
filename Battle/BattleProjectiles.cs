...
namespace ProjectVagabond.Battle.UI
{
    ...
    public class HomingProjectile : BattleProjectile
    {
        ...
        public HomingProjectile(Vector2 startPos, BattleCombatant target, Action onImpact, Color color)
        {
            ...
            _duration = 0.5f + (float)rng.NextDouble() * 0.2f;

            // Control Point: Randomly offset "up and out" to create an arc
            // This creates the "launch" phase before it turns in
            float cpX = rng.Next(-80, 81);
            float cpY = rng.Next(-120, -60);
            ...
            var settings = new ParticleEmitterSettings
            {
                ...
                Lifetime = new FloatRange(0.1f, 0.25f), // Short lifetime for a tight trail
                InitialSize = new FloatRange(4f, 8f),
                EndSize = new FloatRange(0f),
                EmissionRate = 120f, // High rate for a solid streak
                VelocityPattern = EmissionPattern.Radial,
                InitialVelocityX = new FloatRange(0f),
        ...
        public override void Update(GameTime gameTime, Func<BattleCombatant, Vector2> getTargetPos)
        {
            ...
            if (progress >= 1.0f)
            {
                // Force final position to target to ensure visual connection
            ...
            // This simulates the missile finding its target then rocketing towards it
            float t = Math.Clamp(progress, 0f, 1f);
            float easedT = (t == 0) ? 0 : MathF.Pow(2, 10 * (t - 1));

            // Get dynamic target position (in case they are bobbing/moving)
        ...
        {
            var tex = ServiceLocator.Get<SpriteManager>().SoftParticleSprite;
            if (tex != null)
            {
                // Draw the head of the missile, stretched slightly for speed
