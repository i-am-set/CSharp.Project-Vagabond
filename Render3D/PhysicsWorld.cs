...
namespace ProjectVagabond.Physics
{
    ...
    public struct NarrowPhaseCallbacks : INarrowPhaseCallbacks
    {
        ...
        public bool ConfigureContactManifold<TManifold>(int workerIndex, CollidablePair pair, ref TManifold manifold, out PairMaterialProperties material) where TManifold : unmanaged, IContactManifold<TManifold>
        {
            ...
            // --- Publish a collision event for particle effects & nudges ---
            if (manifold.Count > 0)
            {
                // Get contact data once.
                manifold.GetContact(0, out var offset, out var normal, out _, out _);

                // Calculate world position of the contact.
                Vector3 worldPosition;
                if (pair.A.Mobility != CollidableMobility.Static)
                {
                    worldPosition = _simulation.Bodies.GetBodyReference(pair.A.BodyHandle).Pose.Position + offset;
                }
                else
                {
                    worldPosition = _simulation.Statics.GetStaticReference(pair.A.StaticHandle).Pose.Position + offset;
                }

                // Determine if a spark should be created.
                bool isFloorCollision = (pair.A.Mobility == CollidableMobility.Static || pair.B.Mobility == CollidableMobility.Static) && worldPosition.Y < 0.5f;
                ...
                bool shouldSpark = !isFloorCollision && Math.Abs(closingSpeed) > _global.DiceSparkVelocityThreshold;

                // Publish an event for each dynamic, awake body involved in the collision.
                if (pair.A.Mobility == CollidableMobility.Dynamic && _simulation.Bodies.GetBodyReference(pair.A.BodyHandle).Awake)
                {
                    EventBus.Publish(new GameEvents.DiceCollisionOccurred { WorldPosition = worldPosition, BodyHandle = pair.A.BodyHandle, IsSparking = shouldSpark });
                }
                if (pair.B.Mobility == CollidableMobility.Dynamic && _simulation.Bodies.GetBodyReference(pair.B.BodyHandle).Awake)
                {
                    EventBus.Publish(new GameEvents.DiceCollisionOccurred { WorldPosition = worldPosition, BodyHandle = pair.B.BodyHandle, IsSparking = shouldSpark });
                }
            }
