...
namespace ProjectVagabond
{
    ...
    public static class Easing
    {
        private static readonly Dictionary<string, Func<float, float>> _easingFunctions = new Dictionary<string, Func<float, float>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "EaseOutBackSlight", EaseOutBackSlight },
            { "EaseOutBackBig", EaseOutBackBig },
        ...
        public static float EaseOutBackBig(float x)
        {
            const float c1 = 2.7f; // Increased for a more dramatic overshoot
            const float c3 = c1 + 1;
            return 1 + c3 * (float)Math.Pow(x - 1, 3) + c1 * (float)Math.Pow(x - 1, 2);
        }
