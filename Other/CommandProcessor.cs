...
namespace ProjectVagabond
{
    public class CommandProcessor
    {
        ...
        private void HandleShowInventory()
        {
            ...
            if (leader != null)
            {
                Log($"  Spell 1: {leader.Spell1?.CompiledMove.BaseTemplate.MoveID ?? "Empty"} (Used: {leader.Spell1?.TimesUsed ?? 0})");
                Log($"  Spell 2: {leader.Spell2?.CompiledMove.BaseTemplate.MoveID ?? "Empty"} (Used: {leader.Spell2?.TimesUsed ?? 0})");
                Log($"  Spell 3: {leader.Spell3?.CompiledMove.BaseTemplate.MoveID ?? "Empty"} (Used: {leader.Spell3?.TimesUsed ?? 0})");
            }
            else
            {
                Log("  (No Leader)");
