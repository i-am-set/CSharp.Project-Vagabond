...
namespace ProjectVagabond
{
    public class CommandProcessor
    {
        ...
        private void HandleShowInventory()
        {
            ...
            if (leader != null)
            {
                Log($"  Strike: {leader.StrikeMove?.CompiledMove.BaseTemplate.MoveID ?? "Empty"} (Used: {leader.StrikeMove?.TimesUsed ?? 0})");
                Log($"  Alt: {leader.AltMove?.CompiledMove.BaseTemplate.MoveID ?? "Empty"} (Used: {leader.AltMove?.TimesUsed ?? 0})");
            }
            else
            {
                Log("  (No Leader)");
