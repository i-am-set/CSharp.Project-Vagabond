...
namespace ProjectVagabond.Scenes
{
    public class GameOverScene : GameScene
    {
        ...
        public GameOverScene()
        {
            ...
            _titleAnimator = new UIAnimator { EntryStyle = EntryExitStyle.Pop, ExitStyle = EntryExitStyle.Pop, DurationIn = POP_DURATION, DurationOut = POP_DURATION };
            _retryBtnAnimator = new UIAnimator { EntryStyle = EntryExitStyle.Pop, ExitStyle = EntryExitStyle.Pop, DurationIn = POP_DURATION, DurationOut = POP_DURATION };
            _menuBtnAnimator = new UIAnimator { EntryStyle = EntryExitStyle.Pop, ExitStyle = EntryExitStyle.Pop, DurationIn = POP_DURATION, DurationOut = POP_DURATION };
        }
        ...
        private void InitializeUI()
        {
            ...
            int w1 = (int)size1.X + 20;
            int h1 = (int)size1.Y + 8;
            int x1 = (Global.VIRTUAL_WIDTH - w1) / 2;

            var tryAgainButton = new Button(new Rectangle(x1, buttonStartY, w1, h1), text1, font: tertiaryFont) { HoverAnimation = HoverAnimationType.Hop };
            ...
            int w2 = (int)size2.X + 20;
            int h2 = (int)size2.Y + 8;
            int x2 = (Global.VIRTUAL_WIDTH - w2) / 2;

            var menuButton = new Button(new Rectangle(x2, buttonStartY + h1, w2, h2), text2, font: tertiaryFont) { HoverAnimation = HoverAnimationType.Hop };
        ...
        public override void Enter()
        {
            ...
            _titleAnimator.Reset(); _retryBtnAnimator.Reset(); _menuBtnAnimator.Reset();
            ...
            foreach (var button in _buttons) button.ResetAnimationState();
        ...
        private void RestartGame()
        {
            ...
            var loadingTasks = new List<LoadingTask> { new GenericTask("Initializing world...", () => { gameState.InitializeWorld("0"); }) };
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (_currentInputDelay > 0) { _currentInputDelay -= dt; return; }
            ...
            for (int i = 0; i < _buttons.Count; i++)
            {
                ...
                if (animator.IsVisible)
                {
                    ...
                    if (_buttons[i].IsHovered) _selectedButtonIndex = i;
                }
            }
            ...
            {
                if (_selectedButtonIndex != -1) _buttons[_selectedButtonIndex].TriggerClick();
            }
        ...
        protected override void DrawSceneContent(SpriteBatch spriteBatch, BitmapFont font, GameTime gameTime, Matrix transform)
        {
            ...
            {
                Vector2 titleSize = font.MeasureString(_gameOverText);
                Vector2 origin = titleSize / 2f;
                float bobOffset = MathF.Sin((float)gameTime.TotalGameTime.TotalSeconds * 4f) > 0 ? -1f : 0f;
                Vector2 titlePos = new Vector2(Global.VIRTUAL_WIDTH / 2, (Global.VIRTUAL_HEIGHT / 3) + bobOffset) + titleState.Offset;
                spriteBatch.DrawStringSnapped(font, _gameOverText, titlePos, _global.Palette_Rust * titleState.Opacity, 0f, origin, titleState.Scale, SpriteEffects.None, 0f);
            }
            ...
            for (int i = 0; i < _buttons.Count; i++)
            {
                ...
                if (state.IsVisible)
                {
                    ...
                    Matrix buttonTransform = Matrix.CreateTranslation(-center.X, -center.Y, 0) *
                                             ...
                                             Matrix.CreateTranslation(state.Offset.X, state.Offset.Y, 0) * transform;
        ...
        protected override Rectangle? GetFirstSelectableElementBounds() => _buttons.Count > 0 ? _buttons[0].Bounds : null;
    }
}